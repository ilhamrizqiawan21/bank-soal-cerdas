...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar, onToggleMobileMenu }) => {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'admin':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300"><ShieldCheck className="w-3 h-3" /> Admin</span>;
      case 'guru':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300"><BookOpen className="w-3 h-3" /> Guru</span>;
      default:
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300"><GraduationCap className="w-3 h-3" /> Siswa</span>;
    }
  };
    ...
    <header className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 py-3 flex items-center justify-between gap-4 transition-colors">
        ...
        {(currentUser.role === 'admin' || currentUser.role === 'guru') && (
          <form onSubmit={handleSearchSubmit} className="relative hidden md:block max-w-xs w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-sm bg-slate-100 dark:bg-slate-800 border-none rounded-lg focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-slate-100 placeholder-slate-400 outline-none transition-all"
            />
          </form>
        )}
      </div>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <button
          ...
          type="button"
          onClick={toggleTheme}
          className="relative inline-flex items-center justify-center p-2 rounded-xl text-slate-600 hover:bg-slate-100 dark:text-amber-400 dark:hover:bg-slate-800 transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-700"
        ...
        {(currentUser.role === 'admin' || currentUser.role === 'guru') && (
          <button
            ...
            className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 active:scale-95 rounded-lg shadow-sm transition-all"
          >
