...
<div class="container-fluid">
    <div class="stat-card question-form-card" x-data="questionForm()">
        ...
        <form action="{{ route('questions.store') }}" method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-6"><label class="form-label fw-bold">Level Kognitif <span class="text-danger">*</span></label><select name="level_c" class="form-select" x-model="level" @change="loadKKO(level)" required><option value="">Pilih Level</option><option value="L1">L1 - Pengetahuan & Pemahaman (C1-C2)</option><option value="L2">L2 - Penerapan (C3)</option><option value="L3">L3 - Penalaran / HOTS (C4-C6)</option></select><small class="text-muted">L1 = C1-C2, L2 = C3, L3 = C4-C6.</small></div>
                <div class="col-md-6"><label class="form-label fw-bold">KKO <span class="text-danger">*</span></label><select name="kko_id" class="form-select" required><option value="">Pilih KKO</option><template x-for="kko in kkoOptions" :key="kko.id"><option :value="kko.id" x-text="`${kko.verb} (${kko.bloom_level || 'C?'} · ${kko.level})`"></option></template></select></div>
            </div>
