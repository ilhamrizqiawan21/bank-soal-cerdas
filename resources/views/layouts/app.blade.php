...
<head>
    ...
    <style>
        .sidebar .logo-brand-mark{overflow:hidden;padding:5px;background:#fff}
        .sidebar .brand-logo{display:block;width:100%;height:100%;object-fit:contain;border-radius:8px}
        .sidebar.is-collapsed .logo-brand-mark{padding:5px}
    </style>
...
<div id="app" class="app-shell">
    ...
    <aside class="sidebar" :class="{ 'is-open': sidebarOpen, 'is-collapsed': sidebarCollapsed }">
        <div class="sidebar-header">
            <div class="brand-mark logo-brand-mark"><img src="{{ asset('images/android-chrome-512x512.png') }}" alt="Bank Soal" class="brand-logo"></div>
            <div class="brand-text"><span class="brand-name">Bank Soal</span><small class="brand-subtitle">V. 1.0.0</small></div>
            <button type="button" class="sidebar-toggle-btn" @click="toggleSidebar()" aria-label="Collapse sidebar"><i class="fas fa-chevron-left"></i></button>
        </div>
        <nav class="sidebar-nav">
            <p class="sidebar-section-label">Utama</p>
            <a href="{{ route('dashboard') }}" class="nav-item {{ request()->routeIs('dashboard') ? 'active' : '' }}"><i class="fas fa-chart-pie"></i><span class="nav-label">Dashboard</span></a>
            @if(auth()->user()->role === 'admin' || auth()->user()->role === 'guru')
                ...
                <a href="{{ route('subjects.index') }}" class="nav-item {{ request()->routeIs('subjects.*') ? 'active' : '' }}"><i class="fas fa-book"></i><span class="nav-label">Mata Pelajaran</span></a>
        ...
        <header class="topbar">
            ...
            <div class="topbar-actions">
                ...
                @if(in_array(auth()->user()->role, ['admin', 'guru']))
                    ...
                    <div class="dropdown"><button type="button" class="icon-button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Notifikasi" title="Notifikasi"><i class="fas fa-bell"></i>@if($pendingShares->isNotEmpty())<span class="notification-dot"></span>@endif</button><div class="dropdown-menu dropdown-menu-end p-2" style="width:340px"><h6 class="dropdown-header">Undangan Kolaborasi</h6>@forelse($pendingShares as $notif)<div class="d-flex justify-content-between align-items-center gap-2 px-2 py-1"><span class="small flex-grow-1">{{ $notif->message }}</span><form action="{{ $notif->accept }}" method="POST" class="d-inline">@csrf<button type="submit" class="btn btn-sm btn-success" title="Terima"><i class="fas fa-check"></i></button></form><form action="{{ $notif->reject }}" method="POST" class="d-inline">@csrf<button type="submit" class="btn btn-sm btn-danger" title="Tolak"><i class="fas fa-times"></i></button></form></div>@empty<div class="dropdown-item text-muted small">Tidak ada notifikasi.</div>@endforelse</div></div>
                @endif
                @auth
